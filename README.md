...
<img src=".github/readme/banner.svg" alt="경북대 링크 허브 — 학교 사이트 136곳을 한 페이지에, 검색 한 번으로" width="100%" />
...
그래서 자주 찾는 사이트 136곳을 모아 검색 한 번으로 갈 수 있게 만들었습니다.
...
부설연구소와 지원기관이 가장 많고, 그만큼 이름이 낯설고 도메인도 제각각인 자리이기도 합니다.
사실 이 사이트가 가장 쓸모 있는 곳도 여기입니다 — 핵심 시스템 몇 개는 어차피 즐겨찾기 해두기
마련이지만, 부설연구소는 이름조차 가물가물하니까요.
...
| `npm run healthcheck` | 링크 136개가 실제로 살아있는지 확인 |
...
flowchart LR
    subgraph core["항상 성공하는 경로 — DB 없음"]
        A["app/page.tsx (서버 컴포넌트)"] --> B[("lib/links/data.ts<br/>136개 링크, 단일 진실 공급원")]
    end
...
| 파일 | 역할 |
|---|---|
| `lib/links/data.ts` | 링크 136개. 실제로 접속을 확인한 URL만 넣습니다. |
