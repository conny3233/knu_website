...
그 자리에서 확인하고, 승인한 것을 `lib/links/data.ts`에 붙여넣을 TS 코드로 한 번에
내보낼 수 있습니다.

1. `ADMIN_SECRET` 환경변수(로컬 `.env.local`, Vercel 프로젝트 환경변수)에 비밀번호를
   정합니다. 이 값이 없으면 `/admin`은 404입니다 — 존재 자체가 감춰집니다.
2. `/admin`에 접속해 비밀번호로 로그인합니다.
3. 검토할 항목을 체크하고 "전체 생존 확인"으로 URL이 살아있는지 본 뒤, "코드로
   내보내기"를 누르면 `KnuLink` 리터럴 TS 코드가 나옵니다. 복사해서
   `lib/links/data.ts`에 직접 붙여넣고 `id`·`campus`·`keywords`를 다듬은 다음
   커밋합니다(**자동으로 커밋되지 않습니다** — 링크 데이터는 코드가 SSOT이고 사람이
   확인한 것만 들어간다는 원칙은 여기서도 그대로입니다).
4. 붙여넣고 커밋했으면 `/admin`에서 그 항목을 "완료로 표시"합니다. 안 쓰기로 한
   제보는 "반려"로 남겨 목록에서 뺍니다.
